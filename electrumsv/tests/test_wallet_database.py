...
class TestTransactionStore(unittest.TestCase):
    ...
    def test_get(self):
        ...
        # TODO: Test parameter combinations.
...
class TestTxCache(unittest.TestCase):
    ...
    def test_add_then_update(self):
        ...
        cache.add([ (tx_id_1, data, bytedata_1, TxFlags.StateDispatched) ])
        self.assertTrue(cache.is_cached(tx_id_1))
        entry = cache.get_entry(tx_id_1)
        ...
        data = TxData(fee=10, height=88)
        propagate_flags = TxFlags.HasFee | TxFlags.HasHeight
        cache.update([ (tx_id_1, data, None, propagate_flags | TxFlags.HasPosition) ])
        entry = cache.get_entry(tx_id_1)
    ...
    def test_update_or_add(self):
        ...
        # State flags do not get set in an update at this time.
        expected_flags = TxFlags.HasPosition | TxFlags.HasByteData | TxFlags.StateCleared
    ...
    def test_get_transactions(self):
        ...
        cache = TxCache(self.store)
        txs = cache.get_transactions(tx_id)
        for i, tx in enumerate(txs):
            self.assertIsNotNone(tx)
            self.assertEqual(tx_ids[i], tx.txid())
...
class TestXputCache(unittest.TestCase):
    ...
    def test_cache_with_preload(self):
        ...
        for tx_xput, tx_store in ((tx_input, self.txin_store), (tx_output, self.txout_store)):
            tx_store.add_entry(tx_id, tx_xput)

            cache = wallet_database.TxXputCache(tx_store, preload=True)
    ...
    def test_cache_without_preload(self):
        tx_id = os.urandom(10).hex()
        tx_input = wallet_database.TxInput("address_string", "hash", 10, 10)
        tx_output = wallet_database.TxOutput("address_string", 10, 10, False)
        ...
        for tx_xput, tx_store in ((tx_input, self.txin_store), (tx_output, self.txout_store)):
            ...
            cache = wallet_database.TxXputCache(tx_store, preload=False)
            self.assertTrue(tx_id not in cache._cache)
    ...
    def test_cache_delete(self):
        ...
        for tx_xput, tx_store in ((tx_input, self.txin_store), (tx_output, self.txout_store)):
            cache = wallet_database.TxXputCache(tx_store)
            cache.add_entry(tx_id, tx_xput)
            cache.delete_entry(tx_id, tx_xput)
